...
void CAN_Slave_GPIO_Configuration(void)
{
...
void CAN_Slave_NVIC_Configuration(void)
{	
...
static void CAN_Slave_Mode_Configuration(void)
{
...
static void CAN_Slave_Filter_Configuration(void){
...
void CAN_Slave_Configuration(void)
{
	CAN_Slave_GPIO_Configuration();
	CAN_Slave_NVIC_Configuration();
	CAN_Slave_Mode_Configuration();
	CAN_Slave_Filter_Configuration();
}

//打包报文	如果要触发中断的话	data[]里数据要是0xDCBA
...
void USB_LP_CAN1_RX0_IRQHandler(void)					//中断函数名在startup_stm32f10x_xx.s里可看到
{
	...
	if((RXMessage.ExtId == 0x1314) && 					// 比较是否是发送的数据和 ID 
		...
		(RXMessage.DLC == 2) && 
		(RXMessage.Data[1] | RXMessage.Data[0]<<8)==0xABCD)		//判断接受的数据是否为0XABCD
	{
		RX_FLAG = 0;			//接收成功
	}
	else
