#ifndef __BSP_RFID_RC522_H
#define	__BSP_RFID_RC522_H
...
/*********************************** 函数 *********************************************/
void RC522_Init (void);																					//功    能：RC522初始化
char PcdRequest(u8 req_code, u8 * pTagType); 										//功    能：寻卡
char PcdAnticoll(u8 * pSnr);                   									//功    能：防冲撞 读卡号
...
char PcdComMF522 ( uint8_t ucCommand,														//功    能：通过RC522和ISO14443卡通讯
                   uint8_t * pInData, 
                   uint8_t ucInLenByte, 
                   uint8_t * pOutData,
                   uint32_t * pOutLenBit );											
...
void PcdReset(void);																						//功    能：复位RC522
void PcdAntennaOn(void);																				//功    能：开启天线 
void PcdAntennaOff(void);																				//功    能：关闭天线
void M500PcdConfigISOType ( uint8_t ucType );										//功    能：设置RC632的工作方式 
char PcdSelect(unsigned char *pSnr);														//功    能：选定卡片
char PcdAuthState(unsigned char auth_mode,											//功    能：验证卡片密码
									unsigned char addr,
									unsigned char *pKey,
									unsigned char *pSnr);
...
void IC_test (void);																						//功    能：测试RC522并输出IC卡号
